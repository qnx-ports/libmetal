...
{
	CACHE_FLUSH(&__qnx_cache_control, addr, __qnx_get_physical_address(addr, len), len);
}

static inline void __metal_cache_invalidate(void *addr, unsigned int len)
{
	CACHE_INVAL(&__qnx_cache_control, addr, __qnx_get_physical_address(addr, len), len);
}
